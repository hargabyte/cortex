...
Filtering Modes:
  ...
  --keystones    Top N by PageRank (most important entities)
  --bottlenecks  Top N by betweenness (central to call paths)
  --leaves       Top N leaf nodes (no dependents)
...
Examples:
  ...
  cx rank --keystones                         # Top 20 most important entities
  cx rank --keystones --top 10                # Top 10 most important entities
  cx rank --bottlenecks                       # Top 20 by betweenness centrality
  cx rank --leaves                            # Top 20 leaf nodes (no dependents)
...
func runRank(cmd *cobra.Command, args []string) error {
	...
	// Filter leaves if requested (leaves have no dependents)
	if rankLeaves {
	...
	if rankBottlenecks {
		// Sort by betweenness descending for bottlenecks
