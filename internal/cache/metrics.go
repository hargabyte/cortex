...
// EntityMetrics holds cached metrics for an entity.
...
func (c *Cache) SaveMetrics(m *EntityMetrics) error {
	_, err := c.db.Exec(`
...
func (c *Cache) GetMetrics(entityID string) (*EntityMetrics, error) {
	row := c.db.QueryRow(`
...
func (c *Cache) GetAllMetrics() ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetTopByPageRank(n int) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetTopByBetweenness(n int) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetTopByInDegree(n int) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetTopByOutDegree(n int) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetKeystones(threshold float64) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetBottlenecks(threshold float64) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) GetHighlyConnected(threshold int) ([]EntityMetrics, error) {
	rows, err := c.db.Query(`
		SELECT entity_id, pagerank, in_degree, out_degree, betweenness, computed_at
...
func (c *Cache) SaveBulkMetrics(metrics []EntityMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
...
func (c *Cache) DeleteMetrics(entityID string) error {
	_, err := c.db.Exec("DELETE FROM metrics WHERE entity_id = ?", entityID)
	if err != nil {
...
// scanMetrics scans a single row into EntityMetrics.
func scanMetrics(row *sql.Row) (*EntityMetrics, error) {
	var m EntityMetrics
...
// scanMetricsRows scans multiple rows into a slice of EntityMetrics.
func scanMetricsRows(rows *sql.Rows) ([]EntityMetrics, error) {
	var results []EntityMetrics
	for rows.Next() {
		var m EntityMetrics
		var computedAt string
